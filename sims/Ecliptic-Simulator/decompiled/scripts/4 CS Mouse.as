...
{
   var _loc7_ = this._xmouse;
   var _loc6_ = this._ymouse;
   var _loc5_ = Math.sqrt(_loc7_ * _loc7_ + _loc6_ * _loc6_);
   var _loc4_;
   if(_loc5_ > this._c.r)
   {
      _loc4_ = Math.atan2(_loc6_,_loc7_);
      _loc7_ = this._c.r * Math.cos(_loc4_);
      _loc6_ = this._c.r * Math.sin(_loc4_);
   }
   var _loc3_ = {};
   this.StoMH({x:_loc7_,y:_loc6_},_loc3_);
   this.MHtoC(_loc3_,p);
...
p.updateMouseArea = function()
{
   ...
   if(this._traceOn)
   {
      trace("mouse area time: " + (getTimer() - _loc19_) + " ms");
   }
};
