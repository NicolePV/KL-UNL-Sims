...
{
   var depth = this.shell.getDepth();
   if(depth == undefined)
   {
      depth = 7 * this._parent._N + this._id;
   }
   ...
   this.shell = this._parent.createEmptyMovieClip("_obj" + this._id,depth);
   this.instance = this.shell.attachMovie(linkageName,"_obj" + this._id,0,this._initObject);
   this.shell._visible = this._visible;
   if(depth != undefined)
   {
      this.update();
   }
