...
p.setMin = function(arg)
{
   ...
   if(isFinite(x))
   {
      ...
      this._minLabel.labelText = Math.abs(this._min) + latitude;
   }
};
...
p.setMax = function(arg)
{
   ...
   if(isFinite(x))
   {
      ...
      this._maxLabel.labelText = Math.abs(this._max) + latitude;
   }
};
...
p.setValue = function(arg)
{
   ...
   if(isFinite(x))
   {
      ...
      if(this._prec > 0)
      {
         this._valueLabel.labelText = this.toFixed(Math.abs(this._value)) + latitude;
      }
      else
      {
         this._valueLabel.labelText = Math.abs(this._value) + latitude;
      }
      this.positionSlider();
   }
};
