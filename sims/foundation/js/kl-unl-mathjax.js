...
function klunlShowEquation( eqn, msg1, msg2 )  {
  ...
  if ( ( Array.isArray( msg2 ) ) && ( msg2.length >= 2 ) )  {
    ...
    const msgElement2 = document.getElementById(msg2[0]);
    
    if ( msgElement2 )  { msgElement2.textContent = msg2[1]; }
  }
...
function klunlDetabEquation(root) {
  
  // MathJax 3's CHTML output puts tabindex="0" on every <mjx-container> so the
  // container can take focus for its contextual menu. When typeset math is
  // display-only, leaving it in the tab order would put non-controls between
  // the real controls. Dropping to tabindex="-1" takes it out of the Tab
  // sequence while keeping it reachable programmatically, keeping it in the
  // accessibility tree, and leaving the right-click MathJax menu fully working.
