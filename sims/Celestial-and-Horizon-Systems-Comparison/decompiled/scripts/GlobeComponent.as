function GlobeComponentClass()
{
   ...
   this.globe.attachMovie("GlobeComponentWater","water",10);
   this.globe.attachMovie("GlobeComponentLand","land",20);
...
p.updateAxes = function()
{
   ...
   var sf = this._radius * this.globe._yscale / (pc.r * 100);
   ...
   var r = 100 * pc.r / (this._radius * this._yscale);
   if(k8 > 0)
   {
      var mc = this.frontAxis;
      mc.clear();
      mc.lineStyle(this.axisThickness,this.axisColor,this.axisAlpha);
      mc.moveTo(k2,k5);
      mc.lineTo(k2 / sf,k5 / sf);
      var mc = this.backAxis;
      mc.clear();
      mc.lineStyle(this.axisThickness,this.axisColor,this.axisAlpha);
      mc.moveTo(- k2,- k5);
      mc.lineTo((- k2) / sf,(- k5) / sf);
   ...
   var s = this.axisExtent / sf;
   this.ncp = {x:x / sf,y:y / sf,z:z / sf,system:"horizon"};
   this.scp = {x:(- x) / sf,y:(- y) / sf,z:(- z) / sf,system:"horizon"};
   this._parent._parent.PrecessingGlobeNorthPoleAxis.setPoints(this.ncp,{x:x * s,y:y * s,z:z * s,system:"horizon"});
   this._parent._parent.PrecessingGlobeNorthPoleAxis.update();
   this._parent._parent.PrecessingGlobeSouthPoleAxis.setPoints(this.scp,{x:(- x) * s,y:(- y) * s,z:(- z) * s,system:"horizon"});
   this._parent._parent.PrecessingGlobeSouthPoleAxis.update();
   if(this._traceOn)
   {
      trace("globe axis time: " + (getTimer() - start));
   }
};
p.update = function()
{
         ...
         while(j < pl)
         {
            ...
            if(!ibNow)
            {
               if(ibLast)
               {
                  ...
                  var arc = ((angleNow - angleLast) % 6.283185307179586 + 6.283185307179586) % 6.283185307179586;
                  if(arc > 3.141592653589793)
                  {
                     arc = 6.283185307179586 - arc;
               ...
               {
                  mc.lineTo(pt.x * k0 + pt.y * k1 + pt.z * k2,pt.x * k3 + pt.y * k4 + pt.z * k5);
               }
            }
            else if(!ibLast)
            {
