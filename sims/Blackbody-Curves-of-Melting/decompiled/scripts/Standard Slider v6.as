function StandardSliderClassV6()
{
   ...
   this.barMC.onEnterFrameFunc = function()
   {
      ...
      {
         _loc6_ = this._parent.continuousChangeRate * (_loc4_ - this.timeLast);
         ...
         {
            _loc5_ = _loc3_.getIncrementedValueObject(null,- _loc6_);
            if(_loc5_.value <= _loc2_.value)
            {
               this._parent.setValueByValueObject(_loc2_,true);
            }
            else
            {
               this._parent.setValueByValueObject(_loc5_,true);
            }
         }
...
{
   var _loc14_ = this.valueField.text;
...
p.updateFonts = function()
{
   ...
   var _loc3_ = this.labelAndUnitTextFormat;
   var _loc4_ = Math.round(_loc3_.size / 4);
   if(_loc4_ < 3)
   {
      _loc4_ = 3;
   }
   ...
   this.solarSymbolOuterRadius = _loc4_;
   this.solarSymbolInnerRadius = _loc5_;
   this.solarSymbolYPosition = _loc3_.getTextExtent("8").height / 2 - _loc4_;
   this.solarSymbolSpacing = _loc4_ + 2 * _loc5_;
   if(_loc3_.size <= 10)
   {
...
p.displayText = function(textString, options)
{
   ...
   while(_loc8_ < _loc4_.length)
   {
      ...
      _loc14_.push({tf:_loc16_,dy:_loc22_});
      _loc20_ += _loc16_.textWidth;
      _loc8_ = _loc8_ + 1;
   }
   _loc20_ += _loc24_ * (_loc14_.length - 1);
   var _loc19_;
