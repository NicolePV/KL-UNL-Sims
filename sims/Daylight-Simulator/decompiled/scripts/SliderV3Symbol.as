...
p.setMin = function(arg)
{
   ...
   if(isFinite(_loc2_))
   {
      this._min = _loc2_;
      this._scale = (this._max - this._min) / (2 * this._hw);
      this._minLabel.labelText = Math.abs(this._min);
   }
};
p.addProperty("min",p.getMin,p.setMin);
p.getMinLabel = function()
{
   return this._minLabel;
};
p.setMinLabel = function(arg)
{
   this._minLabel1 = arg;
   this._minLabel.labelText += arg;
};
p.addProperty("minLabel1",p.getMinLabel,p.setMinLabel);
p.getMaxLabel = function()
{
   return this._maxLabel;
};
p.setMaxLabel = function(arg)
{
   this._maxLabel1 = arg;
   this._maxLabel.labelText += arg;
};
p.addProperty("maxLabel1",p.getMaxLabel,p.setMaxLabel);
p.getMax = function()
{
   return this._max;
};
...
p.setValue = function(arg)
{
   ...
   if(isFinite(_loc3_))
   {
      ...
      if(this._valueLabel.labelText < 0)
      {
         this._valueLabel.labelText = Math.abs(this._valueLabel.labelText) + this._minLabel1;
      }
      else if(this._valueLabel.labelText > 0)
      {
         this._valueLabel.labelText += this._maxLabel1;
      }
      this.positionSlider();
   }
};
