...
the original Flash material is archived under `fla/`.

```
index.html            KL-UNL scaffold: .app-shell + <kl-unl-masthead> + panels
foundation/           KL-UNL foundation files, copied in unchanged
...
assets/               reused exported bitmap (milkyway.jpg = fla/images/33.jpg)
README.md             this file
...
fla/                  ARCHIVE: the original Flash sim + decompiled source
  milkyWayHabitability001.fla / .swf   original authoring file and movie
  scripts/            decompiled ActionScript (AS1) -- behavior ground truth
  shapes/ images/     exported vector art and bitmaps (reused by the rebuild)
  sprites/ frames/ texts/ symbolClass/ fonts/   other JPEXS export output
  Capture.PNG         screenshot of the running original (layout reference)
  README.md           what each of the above is, and how it maps to the rebuild
```

Nothing in `fla/` is required to run the sim — it is reference/archival material.
See [`fla/README.md`](fla/README.md).
